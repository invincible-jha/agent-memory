...
This document describes the high-level architecture of agent-memory
...
agent-memory uses a decorator-based plugin registry backed by
``importlib.metadata`` entry-points. This allows third-party packages
(including the AgentBrain enterprise edition) to extend the system
...
from agent_memory.core import BaseProcessor  # example base class
