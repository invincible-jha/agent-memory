...
```bash
pip install "agent-memory[agentcore]"
```

### 2. Swap the configuration backend

Replace the default ``Settings`` class with the enterprise version
which reads from your AgentBrain control plane:

```python
# Before (open-source)
from agent_memory.core import Settings
...
The AgentBrain SDK ships with a full set of enterprise plugins that
auto-register when the package is installed:

```python
from agent_memory.plugins.registry import processor_registry
processor_registry.load_entrypoints("agent_memory.plugins")
```
