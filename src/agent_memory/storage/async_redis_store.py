"""Async Redis storage backend — requires redis[asyncio] (guarded import).

Uses the same key structure as the synchronous ``RedisStorage`` so both
backends can operate against the same Redis namespace.

Classes
-------
- AsyncRedisStorage  — redis.asyncio-backed async storage
"""
...
_REDIS_IMPORT_ERROR = (
    "AsyncRedisStorage requires the 'redis' package with asyncio support. "
    "Install it with: pip install redis  or  pip install 'aumos-agent-memory[async-redis]'"
)

# Key prefixes — identical to sync RedisStorage for namespace compatibility
...
class AsyncRedisStorage(AsyncStorageBackend):
    """Async Redis-backed storage using ``redis.asyncio``.
    ...
    client:
        Pre-constructed ``redis.asyncio.Redis`` client (overrides
    ...
    async def save(self, entry: MemoryEntry) -> None:
        ...
        await pipeline.hset(entry_key, mapping=serialised)
        if self._ttl > 0:
            await pipeline.expire(entry_key, self._ttl)
        await pipeline.sadd(_ALL_KEYS_SET, entry.memory_id)
        await pipeline.sadd(_layer_set_key(entry.layer), entry.memory_id)
        await pipeline.execute()

    async def load(self, key: str) -> Optional[MemoryEntry]:
        """Load an entry by memory_id, returning None if not found."""
        data: dict[str, str] = await self._client.hgetall(_entry_key(key))  # type: ignore[union-attr]
        if not data:
            return None
    ...
    async def delete(self, key: str) -> bool:
        ...
        data: dict[str, str] = await self._client.hgetall(entry_key)  # type: ignore[union-attr]
        if not data:
            return False
        layer_value = data.get("layer", "")

        pipeline = self._client.pipeline()  # type: ignore[union-attr]
        await pipeline.delete(entry_key)
        await pipeline.srem(_ALL_KEYS_SET, key)
        if layer_value:
            try:
                layer = MemoryLayer(layer_value)
                await pipeline.srem(_layer_set_key(layer), key)
            except ValueError:
                pass
        await pipeline.execute()
        return True
    ...
    async def search(
        ...
        entries = await self.load_all(layer=layer, limit=limit * 10)
    ...
    async def list_keys(
        ...
        if layer is None:
            all_keys: set[str] = await self._client.smembers(_ALL_KEYS_SET)  # type: ignore[union-attr]
        else:
            all_keys = await self._client.smembers(_layer_set_key(layer))  # type: ignore[union-attr]
        return sorted(all_keys)[:limit]

    async def clear(self, layer: Optional[MemoryLayer] = None) -> int:
        """Remove entries. Returns count deleted."""
        if layer is None:
            all_ids: list[str] = list(await self._client.smembers(_ALL_KEYS_SET))  # type: ignore[union-attr]
        else:
            all_ids = list(await self._client.smembers(_layer_set_key(layer)))  # type: ignore[union-attr]
        ...
        for memory_id in all_ids:
            await pipeline.delete(_entry_key(memory_id))
        await pipeline.execute()

        if layer is None:
            await self._client.delete(_ALL_KEYS_SET)  # type: ignore[union-attr]
            for lyr in MemoryLayer:
                await self._client.delete(_layer_set_key(lyr))  # type: ignore[union-attr]
        else:
            pipeline = self._client.pipeline()  # type: ignore[union-attr]
            for memory_id in all_ids:
                await pipeline.srem(_ALL_KEYS_SET, memory_id)
            await pipeline.delete(_layer_set_key(layer))
            await pipeline.execute()
...
# ------------------------------------------------------------------
# Serialisation helpers (identical logic to sync RedisStorage)
# ------------------------------------------------------------------
