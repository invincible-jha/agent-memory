"""Async in-memory storage backend — dict-based with asyncio.Lock."""
...
class AsyncInMemoryStorage(AsyncStorageBackend):
    """Volatile async dict-based storage backend.
    ...
    async def count(self, layer: Optional[MemoryLayer] = None) -> int:
        """Return the number of stored entries (optimised, no list_keys call)."""
        async with self._lock:
            if layer is None:
    ...
    def __repr__(self) -> str:
        return f"AsyncInMemoryStorage(count={len(self._store)})"


__all__ = ["AsyncInMemoryStorage"]
